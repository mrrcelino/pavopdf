<script lang="ts">
  ...
  function stemFromPath(path: string): string {
    ...
    return fileName.replace(/\.(pdf|docx)$/i, '') || 'output';
  }
  ...
  async function runConvert() {
    ...
    const outFile = await api.saveFileDialog(`${defaultStem}.docx`);
    if (!outFile) return;
    ...
    operationStore.start(opId, 'pdf_to_word' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">PDF to Word</h2>
  </header>
  ...
  >
    {running ? 'Converting...' : 'Convert to Word'}
  </button>
</div>
