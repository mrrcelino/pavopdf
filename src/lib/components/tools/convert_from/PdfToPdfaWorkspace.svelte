<script lang="ts">
  ...
  async function runConvert() {
    ...
    operationStore.start(opId, 'pdf_to_pdfa' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">PDF to PDF/A</h2>
  </header>

  <div class="text-sm text-stone-600 bg-stone-50 border border-stone-200 rounded-lg px-3 py-2 shrink-0">
    Best-effort PDF/A-1b tagging. Adds XMP metadata, sRGB output intent, and removes transparency.
    Not a full conformance validator.
  </div>
  ...
  >
    {running ? 'Converting...' : 'Convert to PDF/A'}
  </button>
</div>
