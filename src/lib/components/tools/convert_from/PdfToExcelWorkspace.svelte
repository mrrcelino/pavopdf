<script lang="ts">
  ...
  function stemFromPath(path: string): string {
    ...
    return fileName.replace(/\.(pdf|xlsx)$/i, '') || 'output';
  }
  ...
  async function runConvert() {
    ...
    const outFile = await api.saveFileDialog(`${defaultStem}.xlsx`);
    if (!outFile) return;
    ...
    operationStore.start(opId, 'pdf_to_excel' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">PDF to Excel</h2>
  </header>
  ...
  >
    {running ? 'Converting...' : 'Convert to Excel'}
  </button>
</div>
