<script lang="ts">
  ...
  const canRun = $derived(filePath !== null && password.trim() !== '' && !running);
  ...
  async function runUnlock() {
    ...
    operationStore.start(opId, 'unlock' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">Unlock PDF</h2>
  </header>
  ...
  {:else}
    ...
    <div class="flex flex-col gap-3 overflow-y-auto">
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Password</span>
        <input
          type="password"
          bind:value={password}
