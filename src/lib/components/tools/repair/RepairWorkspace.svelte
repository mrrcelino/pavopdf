<script lang="ts">
  ...
  async function runRepair() {
    ...
    operationStore.start(opId, 'repair' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">Repair PDF</h2>
  </header>

  <div class="shrink-0 rounded-lg bg-blue-50 border border-blue-200 px-3 py-2 text-sm text-blue-700">
    Attempts to repair corrupted PDFs by re-parsing and normalizing the document structure.
  </div>
