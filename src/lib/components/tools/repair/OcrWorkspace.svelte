<script lang="ts">
  ...
  async function runOcr() {
    ...
    operationStore.start(opId, 'ocr' as Tool);

    try {
      const options: Record<string, string> = {
        language: language.trim() || 'eng',
      };
      if (tesseractPath.trim() !== '') {
        options.tesseract_path = tesseractPath.trim();
      }
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">OCR PDF</h2>
  </header>

  <div class="shrink-0 rounded-lg bg-blue-50 border border-blue-200 px-3 py-2 text-sm text-blue-700">
    Requires Tesseract OCR installed on your system. Download from github.com/UB-Mannheim/tesseract
  </div>
  ...
  {:else}
    ...
    <div class="flex flex-col gap-3 overflow-y-auto">
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Language</span>
        <input
          type="text"
          bind:value={language}
          placeholder="eng"
      ...
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Tesseract Path</span>
        <input
          type="text"
          bind:value={tesseractPath}
