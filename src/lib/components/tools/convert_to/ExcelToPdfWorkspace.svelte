<script lang="ts">
  ...
  async function runConvert() {
    ...
    operationStore.start(opId, 'excel_to_pdf' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">Excel to PDF</h2>
  </header>
  ...
  {#if !filePath}
    ...
    >
      ...
      <span class="text-sm">Click to select an Excel file</span>
    </button>
  {:else}
