<script lang="ts">
  ...
  async function runConvert() {
    ...
    const defaultStem = inputStemFromPath(filePath);
    const outFile = await api.saveFileDialog(`${defaultStem}.pdf`);
    if (!outFile) return;

    if (dirFromPath(outFile) !== dirFromPath(filePath)) {
      error = 'Please choose a location in the same folder as the source file.';
      return;
    }
    ...
    operationStore.start(opId, 'html_to_pdf' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">HTML to PDF</h2>
  </header>

  <div class="text-sm text-amber-700 bg-amber-50 rounded-lg px-3 py-2 shrink-0 border border-amber-200">
    Best-effort text extraction. CSS styling, images, and layout are not preserved.
  </div>
  ...
  {#if !filePath}
    <button
      ...
      onclick={pickFile}
      aria-label="Select an HTML file to convert"
    >
      <svg class="w-10 h-10 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4" />
      </svg>
      <span class="text-sm">Click to select an HTML file</span>
    </button>
  {:else}
