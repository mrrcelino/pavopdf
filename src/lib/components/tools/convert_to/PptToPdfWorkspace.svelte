<script lang="ts">
  ...
  async function runConvert() {
    ...
    operationStore.start(opId, 'ppt_to_pdf' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">PowerPoint to PDF</h2>
  </header>
  ...
  {#if !filePath}
    <button
      ...
      onclick={pickFile}
      aria-label="Select a PowerPoint file to convert"
    >
      <svg class="w-10 h-10 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4" />
      </svg>
      <span class="text-sm">Click to select a PowerPoint (.pptx) file</span>
    </button>
  {:else}
