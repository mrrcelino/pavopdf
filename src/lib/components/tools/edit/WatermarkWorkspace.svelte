<script lang="ts">
  ...
  const canRun = $derived(filePath !== null && text.trim() !== '' && !running);
  ...
  async function runWatermark() {
    ...
    operationStore.start(opId, 'watermark' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">Add Watermark</h2>
  </header>
  ...
  {:else}
    ...
    <div class="flex flex-col gap-3 overflow-y-auto">
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Watermark Text</span>
        <input
          type="text"
          bind:value={text}
      ...
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Opacity ({opacity.toFixed(2)})</span>
        <input
          type="range"
          bind:value={opacity}
          min="0"
      ...
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Rotation (degrees)</span>
        <input
          type="number"
          bind:value={rotation}
          min="-180"
  ...
  >
    {running ? 'Adding Watermark...' : 'Add Watermark'}
  </button>
</div>
