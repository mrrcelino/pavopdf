<script lang="ts">
  ...
  async function runPageNumbers() {
    ...
    operationStore.start(opId, 'page_numbers' as Tool);
...
<div class="flex flex-col gap-4 p-4 h-full overflow-hidden">
  <header class="flex items-center justify-between shrink-0">
    <h2 class="text-lg font-semibold text-stone-800">Add Page Numbers</h2>
  </header>
  ...
  {:else}
    ...
    <div class="flex flex-col gap-3 overflow-y-auto">
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Format</span>
        <input
          type="text"
          bind:value={format}
        ...
        />
        <span class="text-xs text-stone-400">Use {'{n}'} for page number and {'{total}'} for total pages</span>
      </label>
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Position</span>
        <select
          bind:value={position}
          class="px-3 py-2 rounded-lg border border-stone-200 text-sm text-stone-700 focus:outline-none focus:border-teal-400 bg-white"
        >
      ...
      <label class="flex flex-col gap-1">
        <span class="text-sm font-medium text-stone-600">Font Size</span>
        <input
          type="number"
          bind:value={fontSize}
          min="6"
  ...
  >
    {running ? 'Adding Page Numbers...' : 'Add Page Numbers'}
  </button>
</div>
